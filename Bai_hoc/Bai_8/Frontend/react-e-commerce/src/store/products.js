...
const addCartFlagToProducts = (products) =>
...
const useStore = () => {
  ...
  const addProduct = async (product) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(getErrorMessage(payload, "Failed to add product"));
      }
    ...
    try {
      ...
      const productList = Array.isArray(data?.content)
        ...
        : Array.isArray(data)
          ? data
          : [];
      const modifiedData = addCartFlagToProducts(productList);
  ...
  const getProductsByPage = async (page = 0, size = 9) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          getErrorMessage(data, "Error fetching paginated products")
        );
      }

      if (!Array.isArray(data?.content)) {
        throw new Error("Product list response is invalid");
      }

      const productsWithFlag = addCartFlagToProducts(data.content);
      ...
      toast.error(error.message || "Error fetching paginated products");
      return null;
    }
  };
