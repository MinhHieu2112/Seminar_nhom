...
export class AnalyticsController {
  ...
  async handleTaskCreated(@Payload() task: any) {
    await this.analyticsService.recordTaskEvent(task, 'created');
  }

  @EventPattern('task.status.updated')
  async handleTaskStatusUpdated(@Payload() task: any) {
    if (task.status === 'done') {
      await this.analyticsService.recordTaskEvent(task, 'completed');
    } else {
      await this.analyticsService.recordTaskEvent(task, 'updated');
    }
  }

  @EventPattern('task.allocated')
  async handleTaskAllocated(@Payload() allocation: any) {
    await this.analyticsService.recordAllocationEvent(allocation);
  }
}
